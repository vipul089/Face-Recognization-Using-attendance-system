...
def train():
    ...
    recognizer.save('backend/trainningData.yml')
    cv2.destroyAllWindows()
    messagebox.showinfo("Confirm","Your Data Successfully Trained")
...
class dashboard:
    def __init__(self, root, empname):
        ...
        self.mg_image = ImageTk.PhotoImage(file="images/manage.jpg")
        self.tr_image = ImageTk.PhotoImage(file="images/trainer.jpg")
        ...
        self.dashboard.place(x=0, y=171, relwidth=1, height=43)

        self.bl = Label(self.dashboard, text="          ", fg='#472074', font='Arial 18 bold', bg='#93AAAA' )
        self.bl.grid(row=0, column=0, pady=4 )
        ...
        self.act.grid(row=0, column=3, padx=250, )
        ...
        self.mu_btn = Button(self.root, image=self.mg_image, bd=0, command=manage)
        self.mu_btn.place(x=75, y=250, )

        self.tr_btn = Button(self.root, image=self.tr_image, bd=0, command=train)
        self.tr_btn.place(x=500, y=250, )

        self.cp_btn = Button(self.root, image=self.cp_image, bd=0, command=cp )
        self.cp_btn.place(x=925, y=250, )

        self.re_btn = Button(self.root, image=self.re_image, bd=0, command=re )
        self.re_btn.place(x=75, y=480, )

        self.lo_btn = Button(self.root, image=self.lo_image, bd=0, command=logout)
        self.lo_btn.place(x=500, y=480, )
