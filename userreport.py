...
def search():
    ...
    if (v.get() == 1):
        remove_all()
        find_time_query = cur.execute("Select * from attdata where id= ? AND date = ?", (eid,cal1.get()))
        attdata = find_time_query.fetchall()
        for i in attdata:
            tv.insert('', 'end', values=i, tags='T')
    elif(v.get()==2):
        remove_all()
        find_time_query = cur.execute("Select * from attdata where id= ? AND date BETWEEN ? AND ?", (eid, cal2.get(), cal3.get()))
        attdata = find_time_query.fetchall()
        for i in attdata:
            tv.insert('', 'end', values=i, tags='T')
    conn.commit()
...
class ChangePass:
    def __init__(self, root, empname):
        ...
        radio_btn1 = Radiobutton(self.Login_frame, text="Date", font='Arial 14 bold', value=1, command=deen, variable=v)
        radio_btn1.grid(row=0, column=0)
        cal1 = DateEntry(self.Login_frame, width=12, background='White',foreground='black',  maxdate=today, date_pattern="DD-MM-YYYY", textvariable=cal1t)
        cal1.config(state="disabled")
        ...
        radio_btn2 = Radiobutton(self.Login_frame, text="Start Date", font='Arial 14 bold', value=2, command=deen, variable=v)
        radio_btn2.grid(row=0, column=3)
        cal2 = DateEntry(self.Login_frame, width=12, background='White',foreground='black', maxdate=today, date_pattern="DD-MM-YYYY",  textvariable=cal2t )
        cal2.config(state="disabled")
        ...
        cal3 = DateEntry(self.Login_frame, width=12, background='White',foreground='black', maxdate=today, date_pattern="DD-MM-YYYY",  textvariable=cal3t )
        cal3.config(state="disabled")
